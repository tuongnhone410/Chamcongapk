...
export default function SettingsPage() {
  ...
  // Kiểm tra xem dữ liệu tạm thời có khác biệt so với dữ liệu gốc đã lưu không
  const hasChanges = useMemo(() => {
    if (!settings || !localSettings) return false;
    return JSON.stringify(settings) !== JSON.stringify(localSettings);
  }, [settings, localSettings]);

  // Trả về Skeleton Loader đẹp mắt thay vì trả về màn hình trống
  ...
  const handleMoneyInput = (key: keyof AppSettings, val: string) => {
    ...
    if (key === 'baseMonthlySalary') {
      const calculatedHourly = Math.round(num / 208); // 208h = 26 ngày công chuẩn * 8 tiếng
      setLocalSettings({
        ...localSettings,
  ...
  const handleSave = async () => {
    ...
    // --- BƯỚC VALIDATE DỮ LIỆU TRƯỚC KHI LƯU ---
      ...
      <header className="flex items-center justify-between sticky top-0 z-20 bg-zinc-950/90 py-4 backdrop-blur-md border-b border-zinc-900/50">
        ...
        <Button 
          ...
          className={cn(
            ...
            hasChanges 
              ? "bg-primary text-black hover:bg-primary/90 animate-shimmer" 
